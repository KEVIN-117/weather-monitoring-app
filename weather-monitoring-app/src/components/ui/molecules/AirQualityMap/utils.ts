...
export interface AirQualityData {
  ...
  export const getUserLocation = (onPositionGathered: (position: GeolocationPosition) => void, onPositionGatherError: (error: GeolocationPositionError) => void) => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(onPositionGathered, onPositionGatherError);
    } else {
      onPositionGatherError({ message: "Unable to access your location. Please enter the coordinates manually" } as GeolocationPositionError);
    }
  };
  
  export const getAirQuality = async (lat: string, lon: string, setValueOfAir: (airData: AirQualityData) => void, setComponentOfAir: (airData: AirQualityData) => void, onError: (e: Error) => void) => {
    try {
  ...
  export const setValueOfAir = (airData: AirQualityData, airQuality: HTMLSpanElement | null, airQualityStat: HTMLSpanElement | null) => {
  ...
  export const setComponentOfAir = (airData: AirQualityData, componentsEle: NodeListOf<HTMLSpanElement>) => {
  ...
  export const handleSearchClick = (latInp: HTMLInputElement | null, lngInp: HTMLInputElement | null, getAirQualityFn: (lat: string, lon: string) => void) => {
    if (latInp && lngInp) {
